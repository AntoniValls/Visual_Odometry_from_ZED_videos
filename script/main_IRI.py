...
if __name__ == '__main__':
    ...
    thresholds = [100, 256, 512]
    max_depths = [20, 50, 100, 250, 300, 1000, 5000]

    for rectify in [False, True]:
        config['parameters']['rectify'] = rectify
        for i in range(len(thresholds)):
            ...
            for j in range(len(max_depths)):
                ...
                try: 
                    ...
                    np.savetxt(os.path.join(save_dir, f"{config['parameters']['detector']}_t{config['parameters']['threshold']}_d{config['parameters']['max_depth']}_r{rectify}_cSGBM_nR.txt"), positions, fmt="%.16f")
