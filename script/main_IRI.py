    ...
    for i in range(len(thresholds)):
        ...
        for j in range(len(max_depths)):
            ...
            try: 
                ...
                np.savetxt(os.path.join(save_dir, f"{config['parameters']['detector']}_{config['parameters']['depth_model']}_threshold{config['parameters']['threshold']}_maxdepth{config['parameters']['max_depth']}_rectify{config['parameters']['rectified']}_smoothed.txt"), positions, fmt="%.16f")
